import './style.css'
import { get, post, del, TData } from './http'
...
const getFormElement = (event) => (elementName: any) => {
  return event.target.elements[elementName]
}

type TElementType = {
  image: (data: { src: string; alt: string }) => HTMLTableDataCellElement
  text: (value: string) => HTMLTableDataCellElement
  color: (value: string) => HTMLTableDataCellElement
}
...
function createTableRow(data: TData) {
  const elements = [
  ...
  elements.forEach((element) => {
    const td = elementTypes[element.type as keyof TElementType](element.value)
    tr.appendChild(td)
  })
